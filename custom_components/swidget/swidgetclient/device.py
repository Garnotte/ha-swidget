...
class SwidgetDevice:
    ...
    async def process_state(self, state):
        ...
        try:
            ...
            pass
        """
        2022-06-28 14:53:13 ERROR (MainThread) [custom_components.swidget.swidgetclient.device] 
        Self.assemblies: {'host': <custom_components.swidget.swidgetclient.device.SwidgetAssembly object at 0xffff862b6280>, 
                          'insert': <custom_components.swidget.swidgetclient.device.SwidgetAssembly object at 0xffff8f0b5520>}
        ...
        for assembly in self.assemblies:
            ...
            for id, component in self.assemblies[assembly].components.items():
                _LOGGER.debug(f"components: {id} {component.__dict__}")
                _LOGGER.debug(f"component.function: {component.functions}")
                try:
                    _LOGGER.debug(f"Get State: {state[assembly]['components'][id]}")
                    component.functions.update(state[assembly]["components"][id])
                except:
                    pass
        self._last_update = int(time.time())
        _LOGGER.debug(f"List of dict: {self.__dict__}")
        a = self.assemblies['host'].__dict__
        b = self.assemblies['insert'].__dict__

        _LOGGER.debug(f"List of host dict: {a}")
        _LOGGER.debug(f"List of insert dict: {b}")
    ...
    async def ping(self):
        ...
        try:
            ...
            # Raise the exception if unable to connect to the Swidget device
    ...
    #def get_buttonLevel(self, timer_id=0):
    #    """Get the timer button level."""
    #    button_level = 0
    #    return_dict = {}
    #    for id, properties in self.assemblies['host'].components.items():
    #        try:
    #            return_dict[f"timer_{id}"] = properties.functions['timer']['buttonLevel']
    #        except KeyError: # Hits this when there is no power metering
    #            return None
    #        return return_dict
    #    return self.assemblies['host'].components['0'].functions['timer']['buttonLevel']
    ...
    def features(self) -> List[str]:
        """Return a set of features that the device insert supports."""
        try:
